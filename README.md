...
- **Moderator** — ครูที่ได้สิทธิ์นี้จากหน้าสิทธิ์ผู้ดูแล: ยังเป็น Teacher ทุกอย่าง และแก้ไขได้เพิ่ม 2 หน้า — **วิชาเสริม** (`/admin/subjects`) และ **ตารางเรียนทั้งปี** (`/admin/schedule`) · หน้า admin อื่นเข้าไม่ได้
...
- ให้ได้ 2 ระดับ (`admin_grants.role`): **ผู้ดูแล** (`admin`) หรือ **Moderator** (`moderator`) —
  จะเปลี่ยนระดับให้ถอนแล้วให้ใหม่
- Moderator เป็น flag ที่ `authz.ts` อ่านจาก DB (ไม่ฝังใน JWT) · สองหน้าของ Moderator อยู่ใน
  route group `src/app/admin/(catalog)/` ซึ่ง layout ใช้ `requireCatalogEditor()` ส่วนหน้า admin
  อื่นทั้งหมดอยู่ใน `src/app/admin/(admin)/` ซึ่ง layout ยังเป็น `requireRole('admin')` —
  **หน้า admin ใหม่ต้องวางใน `(admin)/`** ไม่เช่นนั้นจะไม่มี layout ใดกันไว้
- ทุกครั้งที่ให้/ถอน บันทึกลง `activity_logs` (`grant_admin` / `revoke_admin` /
  `grant_moderator` / `revoke_moderator`)
