...
`src/lib/evaluate.ts` เป็น **pure function** ทดสอบแยกจาก UI ได้ (`npm test` — 11 เคส):
...
src/
  app/
    ...
    homeroom/               ห้องที่ปรึกษา — ครูเห็นห้องตัวเอง, admin เลือกห้อง
                            ได้ทุกห้อง + ออกรายงาน PDF
...
5. **จัดนักเรียนเข้าวิชา** → "เพิ่มรอบเรียน" → เลือกหมวด/วิชา → ตั้งชื่อรอบ + ห้องเรียน →
...
6. **เช็คชื่อ** → เลือกวัน → เลือกรอบเรียน (บอกห้องที่เรียน) → กด "มาทั้งหมด" แล้วแก้เฉพาะคนที่ไม่มา
